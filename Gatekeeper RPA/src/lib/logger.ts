...
// Client-side compatible logger for Next.js
// Only use Node.js modules on server side
let winston: any;
let createHash: any;
...
// Create logger instance only on server side
let logger: any;
...
// Only define server-side logging classes when on server
if (typeof window === 'undefined') {
...
// Utility functions - server side only
...
// Export all logger instances for compatibility
export { logger, SecurityLogger, PerformanceLogger, AuditLogger, requestLogger, securityLogger };